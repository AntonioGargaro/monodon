# @antoniog/typescript-nx-imports-plugin
...
"plugins": [
      {
        "name": "@antoniog/typescript-nx-imports-plugin",
        "externalFiles": [
          {
...
Build the `@antoniog/typescript-nx-imports-plugin` with `yarn nx build typescript-nx-imports-plugin --watch`. Navigate to the `dist/packages/typescript-nx-imports-plugin` directory and run `npm link`. Navigate back to the root of this project and run `npm link @antoniog/typescript-nx-imports-plugin`.
